...
function calculaVINT(suave,velfile,velfilein,PATH);
...
velfile=strcat([PATH,velfile,char,'.bin']);
velfilein=strcat([PATH,velfilein,char,'.bin']);
...
%zfinal (3000), with sample interval of 10. Then nzint will simply count
...
if exist('ini.txt','file');
   ...
   fclose(fid);
   zini=interp1(ini(:,1),ini(:,2),pvec,'linear','extrap');
...
for p=pvec
    ...
    ii=find(dado(1,:)>zini(j));
    if isempty(ii);
        error(strcat(['No pick below initial depth level at x = ',num2str(p)]));
    end
    pick=[zini(j) vini(j);dado(1,ii)' dado(2,ii)']';
    npick=length(pick);
    ...
    for prof=zini0:dz:zfinal
        ...
        for k=2:npick
            if velant == 1;
                yint(m)=interp2(xx,zz,veloINTant,p,prof);
            end
...
if velant == 1;
   veloINTnew=veloINTant;
end
for j=1:nnp;
    ii=find(zinterp>=newzini(j));
    veloINTnew(ii,j)=veloINT(ii,j);
end
